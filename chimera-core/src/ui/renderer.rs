...
impl Renderer {
    ...
    fn draw_header<D>(&self, display: &mut D, nav: &ChainNav)
    where
        D: DrawTarget<Color = Rgb565>,
    {
        ...
        let _ = Text::new(node.name, Point::new(x, y), bright).draw(display);
    ...
    fn draw_visualization<D>(&self, display: &mut D, page: PageId)
    where
        D: DrawTarget<Color = Rgb565>,
    {
        match page {
            ...
            PageId::Folder => self.draw_folder_viz(display),
    ...
    fn draw_params<D>(&self, display: &mut D, page: PageId)
    where
        D: DrawTarget<Color = Rgb565>,
    {
    ...
    fn draw_cell_grid<D>(&self, display: &mut D, page: PageId)
    where
        D: DrawTarget<Color = Rgb565>,
    {
        let labels = page.encoder_labels();
