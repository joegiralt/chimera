...
fn cpu_budget_steals_or_refuses() {
    ...
    for i in 0..4 {
        assert!(matches!(
            a.note_on(0, Poly, n(60 + i), modal, fx),
            Alloc::Voice(_)
        ));
    }
    assert_eq!(
        a.note_on(1, Poly, n(70), modal, fx),
        Alloc::Voice(0),
        "steals the oldest"
    );
...
fn random_play_keeps_the_pool_invariants() {
        ...
        for step in 0..2_000 {
            ...
            match rng.below(4) {
                0 | 1 => {
                    ...
                    let before: Vec<_> = a
                        .slots()
                        .iter()
                        .map(|s| (s.part(), s.note(), s.held()))
                        .collect();
                    let cost = COSTS[part as usize % 3];
                    if let Alloc::Voice(v) =
                        a.note_on(part, modes[part as usize], n(note), cost, FX)
                    {
                        ...
                        if let (Some(p), _, was_held) = before[v] {
                            assert!(
                                modes[p as usize] == Poly || p == part,
                                "{ctx}: mono voice of part {p} stolen"
                            );
                            ...
                            let tail_ringing = before.iter().any(|&(q, _, h)| {
                                q.is_some_and(|q| modes[q as usize] == Poly) && !h
                            });
                            assert!(
                                !(stolen && was_held && tail_ringing),
                                "{ctx}: held voice stolen over a tail"
                            );
                        }
                    }
                }
            ...
            for &(p, nn, v) in &held {
                ...
                assert_eq!(
                    ...
                    (Some(p), Some(n(nn)), true),
                    "{ctx}"
                );
            }
            assert_eq!(
                a.slots().iter().filter(|s| s.held()).count(),
                held.len(),
                "{ctx}"
            );
            ...
            for p in 0..6u8 {
                if modes[p as usize] == Mono {
                    assert!(
                        a.slots().iter().filter(|s| s.part() == Some(p)).count() <= 1,
                        "{ctx}"
                    );
                }
            }
    ...
    assert!(
        steals > 0 && refusals > 0,
        "steals {steals}, refusals {refusals}"
    );
}
