...
fn test_fm_algo_change_mid_note() {
    let (before_rms, after_rms, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Fm; p.fm.op_level = [0.5, 0.3, 0.3, 1.0]; },
        |p| { p.fm.algorithm = 7; }, // switch to additive
        8, 8,
    );
    let diff: f32 = before.iter().zip(after.iter())
        .map(|(a, b)| (a - b).abs()).sum::<f32>() / before.len() as f32;
...
fn test_fm_feedback_change_mid_note() {
    ...
    assert!(
        ...
        "feedback change should alter harmonics: before={} after={}", h_before, h_after
    );
}
...
fn test_fm_modulator_depth_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Fm; p.fm.algorithm = 4; p.fm.op_level[1] = 1.0; },
        |p| { p.fm.op_level[0] = 0.8; }, // crank modulator
        8, 8,
    );
    let f0 = 261.6;
    let h_before = harmonic_energy(&before, f0);
    ...
    assert!(
        h_after > h_before,
        "increasing modulator should add harmonics: before={} after={}", h_before, h_after
    );
}
...
fn test_filter_cutoff_sweep_mid_note() {
    let (before_rms, after_rms, _, _) = render_with_param_change(
        ...
        |p| { p.filter.cutoff.set(200.0); }, // close the filter
        8, 8,
    );
    assert!(
        after_rms < before_rms * 0.5,
        "closing filter should reduce level: before={} after={}", before_rms, after_rms
    );
}
...
fn test_filter_resonance_mid_note() {
    ...
    assert!(
        ...
        "resonance should boost cutoff freq: before={} after={}", peak_before, peak_after
    );
}
...
fn test_drive_amount_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Fm; p.drive.drive.set(0.0); p.drive.mix.set(1.0); },
        |p| { p.drive.drive.set(0.9); },
        8, 8,
    );
    let f0 = 261.6;
    let h_before = harmonic_energy(&before, f0);
    ...
    assert!(
        h_after > h_before,
        "drive should add harmonics: before={} after={}", h_before, h_after
    );
}
...
fn test_folder_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Fm; p.folder.fold.set(0.0); },
        |p| { p.folder.fold.set(0.8); p.folder.mix.set(1.0); },
        8, 8,
    );
    let f0 = 261.6;
    let h_before = harmonic_energy(&before, f0);
    ...
    assert!(
        h_after > h_before,
        "folder should add harmonics: before={} after={}", h_before, h_after
    );
}
...
fn test_ks_body_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Modal; p.modal.mode = 0; p.modal.ks_body = 0.0; },
        |p| { p.modal.ks_body = 0.8; },
        8, 8,
    );
    let diff: f32 = before.iter().zip(after.iter())
        .map(|(a, b)| (a - b).abs()).sum::<f32>() / before.len() as f32;
...
fn test_ks_stiffness_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Modal; p.modal.mode = 0; p.modal.ks_stiffness = 0.0; },
        |p| { p.modal.ks_stiffness = 0.7; },
        8, 8,
    );
    let diff: f32 = before.iter().zip(after.iter())
        .map(|(a, b)| (a - b).abs()).sum::<f32>() / before.len() as f32;
...
fn test_ks_feedback_mid_note() {
    let (before_rms, after_rms, _, _) = render_with_param_change(
        |p| { p.engine = EngineType::Modal; p.modal.mode = 0; p.modal.ks_feedback = 0.0; },
        |p| { p.modal.ks_feedback = 0.9; },
        8, 16,
    );
    ...
    assert!(
        after_rms > 0.001,
        "feedback should help sustain: after_rms={}", after_rms
    );
}
...
fn test_ks_brightness_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Modal; p.modal.mode = 0; p.modal.brightness = 0.1; },
        |p| { p.modal.brightness = 0.9; },
        4, 8,
    );
    let f0 = 261.6;
    let h_before = harmonic_energy(&before, f0);
    ...
    assert!(
        ...
        "brightness should change harmonics: before={} after={}", h_before, h_after
    );
}
...
fn test_modal_decay_mid_note() {
    let (before_rms, after_rms, _, _) = render_with_param_change(
        |p| { p.engine = EngineType::Modal; p.modal.mode = 1; p.modal.decay = 0.8; },
        |p| { p.modal.decay = 0.1; }, // shorten decay dramatically
        8, 16,
    );
    // After shortening decay, the sound should die faster
    assert!(
        after_rms < before_rms || after_rms < 0.01,
        "reducing decay should quiet the sound: before={} after={}", before_rms, after_rms
    );
}
...
fn test_modal_brightness_mid_note() {
    let (_, _, before, after) = render_with_param_change(
        |p| { p.engine = EngineType::Modal; p.modal.mode = 1; p.modal.brightness = 0.1; },
        |p| { p.modal.brightness = 1.0; },
        4, 8,
    );
    let f0 = 261.6;
    let h_before = harmonic_energy(&before, f0);
    ...
    assert!(
        (h_before - h_after).abs() > 0.0001,
        "modal brightness should change spectrum: before={} after={}", h_before, h_after
    );
}
...
fn test_volume_mid_note() {
    let (before_rms, after_rms, _, _) = render_with_param_change(
        |p| { p.engine = EngineType::Fm; p.volume.set(0.8); },
        |p| { p.volume.set(0.1); },
        8, 8,
    );
    assert!(
        after_rms < before_rms * 0.3,
        "reducing volume should reduce level: before={} after={}", before_rms, after_rms
    );
}
